...
class CustomerChart(BaseModel):
    ...
    def model_post_init(self, __context: Any):
        self.dob_time = self.standard_birth_datetime(self.dob_time)
    ...
    @classmethod
    def standard_birth_datetime(cls, dob_time: Union[str, datetime]) -> datetime:
        try:
    ...
    def get_dayun(self, gender: int, num: int):
        ...
        step = 1 if gender == self.ba_zi_generator.year_pillar.di_zhi.positive else -1
        ...
        for i in range(num):
            ...
            TianGan.get_tian_gan_by_index(tg_dayun_index).get_shi_shen(
                day_master=self.ba_zi_generator.day_pillar.tian_gan
            ),
            DiZhi.get_di_zhi_by_index(dz_dayun_index).get_shi_shen(
                day_master=self.ba_zi_generator.day_pillar.tian_gan
            )
    ...
    def _format_results(self, item):
        caption = [item.wu_xing.base.name]
